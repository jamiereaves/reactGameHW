...
class App extends Component {
  ...
  state = {
    ...
    score: 0,
    highScore: 0,
    message: "Click on any image to earn a point. If you can make it to 26 points, you keep the beans. Click an image twice, though, & your game is over...when you play the game of beans, you win, or you die. There is no middle ground!"
  };
  ...
  clickHandler = (x) => {
    ...
    if (targetCard[0].clicked===false && this.state.score===25){
      this.setState({highScore: 20});
      this.setState({message: "Congratulations! Click any image to start again!!"});
    ...
    //check to see if the card has been clicked
    else if (targetCard[0].clicked===false){
      ...
      //if it has, reset the values of all "clicked" cards, reset score to 0, & change message state
        ...
        this.setState({message: "You already chose " + targetCard[0].name +"...click any image to start again!"}, function () {
          console.log(this.state.message);
        })
